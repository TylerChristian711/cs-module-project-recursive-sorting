...
def merge(arrA, arrB):
    ...
    for i in range(elements):
        ...
        if a >= len(arrA):
            # set the merged array's i index equal to the b index in array B
            merged_arr[i] = arrB[b]
            # increment B
            b += 1
        elif b >= len(arrB):
            ...
            # set the merged array's i index equal to the b index in array A
            merged_arr[i] = arrA[a]
            # increment A
            a += 1
        else:
...
def merge_in_place(arr, start, mid, end):
    ...
    # set up inital values to keep track of progress through array
    left_index = 0
    right_index = 0
    sorted_index = start

    # iterate thought both copies until we have no elements left in one of them
